...
def preprocess(
    ...
    resized = cv2.resize(
        np_image, (IMG_SIZE_W, IMG_SIZE_H), cv2.INTER_CUBIC
    )
...
def predict(img_in: np.ndarray) -> np.ndarray:
    ...
    predictions = model.run(
        output_names=[outputModelName],
        input_feed={inputModelName: img_in},
    )[0]
...
def postprocess(
    ...
    infer_shape = (IMG_SIZE_H, IMG_SIZE_W)
...
image = Image.open(r"D:\tmp\rad2\all_aspects_rad_2_quality_2\aspects-all-rad-2-quality-2-id-2.png")
